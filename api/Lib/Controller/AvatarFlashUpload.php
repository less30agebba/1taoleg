...
class Controller_AvatarFlashUpload extends Controller_Base{
    ...
    function rectavatarAction() {
        ...
        foreach( $avatarsize as $key => $size ){
            ...
            $writebyte = dd_file_put( $avatarrealpath, $avatarcontent, LOCK_EX );
            if( $writebyte <= 0 ){
                $success = 0;
                return '<root><message type="error" value="-2" /></root>';
                break;
		...
		if(FUJIAN_FTP==1){
			...
			include(DDROOT.'/comm/ftp.class.php');
			...
			foreach($avatarsize as $v){
				$avatar_path=DDROOT.'/upload/avatar/'.$dir1 . '/' . $dir2 . '/' . substr($uid, -2) . "_avatar_".$v.".jpg";
				$re=$ftp->make_dir_file($avatar_path,0);
			}
		}
	...
	public function get_avatar_filepath($uid, $size = 'big', $type = '') {
		...
		return  $dir1.'/'.$dir2.'/'.substr($uid, -2).$typeadd."_avatar_$size.jpg";
	}
