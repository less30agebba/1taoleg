...
if (!function_exists('pageft')) {
	...
	function pageft($totle, $displaypg = 20, $url = '',$wjt=0, $shownum = 1, $showtext = 1, $showselect = 0, $showlvtao = 7) {
		...
		if($displaypg<0){
			$displaypg=abs($displaypg);
			if ($wjt == '1') {
				$html="<span class='current'>".$page."</span>";
				if($totle==$displaypg){
					$html.="<a href='$url-".($page+1).".html'>></a>";
				}
				if($page>1){
					$html="<a href='$url-1.html'>首页</a><a href='$url-".($page-1).".html'><</a>".$html;
				}
			}
			else{
				$html="<span class='current'>".$page."</span>";
				...
				if($page>1){
					$html="<a href='$url=1'>首页</a><a href='$url=".($page-1)."'><</a>".$html;
				}
			}
			return $html;
		}
