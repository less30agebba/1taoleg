...
function limit_input($name,$value=DEFAULTPWD,$width='150',$pwd=1){
	...
	return $s='<input style="width:'.$width.'px" type="'.$type.'" _name="'.$name.'" id="'.$id.'" readonly="readonly" class="disabled" value="'.$value.'"/><input type="checkbox" title="激活修改" onClick="if($(this).prop(\'checked\')==\'checked\' || $(this).prop(\'checked\')==true){$(\'#'.$id.'\').attr(\'readonly\',false).removeClass(\'disabled\').attr(\'name\',$(\'#'.$id.'\').attr(\'_name\')).val(\'\');}else{$(\'#'.$id.'\').attr(\'readonly\',\'readonly\').addClass(\'disabled\');}"  />';
}
...
function reg_taobao_url($url){
    if(preg_match('/(taobao\.com|tmall\.com|tmall\.hk|95095\.com)/',$url)==1){
		return 1;
	}
	else{
	    return 0;
...
function is_kong($v=''){
	if($v=='' || $v===0 || $v==='0' || $v===0.00 || strpos($v,'1970-01-01')!==false || strpos($v,'-0001-11-30')!==false){
		return '——';
	}else{
		return $v;
	}
