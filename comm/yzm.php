...
class Utils_Caption {
	...
	var $TFontAngle = array (
		-20,
		20
	); //旋转角度

	var $Chars = "2345678abcdefghgkmnpqrstuvwxyz"; //验证码范围（字母数字）
	...
	public function Draw() ///画图
	{
		...
		foreach ($codes as $v) ///逐个画字符
			...
			$a=imagettftext($image, $v[0], $v[1], $basex, $bound['height'], $color,  DDROOT.'/comm/'.$v[4], $v[3]);
	...
	*/
	private function _calculateTextBox($font_size, $font_angle, $font_file, $text) {
		$box = imagettfbbox($font_size, $font_angle, DDROOT.'/comm/'.$font_file, $text);
		...
		return array (
			'left' => ($min_x >= -1
		) ? -abs($min_x +1) : abs($min_x +2), 'top' => abs($min_y), 'width' => $max_x - $min_x, 'height' => $max_y - $min_y, 'box' => $box);
	}
	...
	private function _getColor($color) //#ffffff
	{
		...
		), hexdec($color[3] . $color[4]), hexdec($color[5] . $color[6]));
	}
