    ...
    public function getSend ($paramArr)
    {
		...
		if($error==''){
			...
			elseif(HASAPI==1 && TAOTYPE==1){ //有数据调用api的
				...
				//$uu='ddurl='.urlencode(URL). '&' . $strParam;
				//$this->_systemParam['url'] = DD_YUN_URL.'/api/?'.$uu .'&ddsign='.urlencode(md5(DDYUNKEY.$uu.DDYUNKEY));
				$this->_systemParam['url']='';
			}
			
			if($this->_systemParam['url']!=''){
			...
			if($paramArr['method']=='taobao.tbk.rebate.order.get'){
				$result=preg_replace('/"num_iid":(\d+)/','"num_iid":"\1"',$result);
				$result=preg_replace('/"trade_id":(\d+)/','"trade_id":"\1"',$result);
				$result=preg_replace('/"trade_parent_id":(\d+)/','"trade_parent_id":"\1"',$result);
			}
            //返回结果
            return $result;
		}
