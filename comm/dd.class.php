...
function set_constants($table) {
	...
	public function rebate($user,$trade,$shijian=0){ //user数组包含会员名，会员id，会员等级，推荐人  trade数组包含订单全部信息，带有id号说明是找回订单
		...
		if(isset($trade['trade_id'])){
			...
			if($trade['commission']==0){
				...
				$trade['commission']=(float)str_replace(',','',$trade['commission']);
			}
			
			if(!isset($trade['fxje']) || $trade['fxje']==0){
				$trade['fxje']=fenduan($trade['commission'], $this->webset['fxbl'], $user['type']);
				$trade['fxje']=number_format($trade['fxje'],2);
				$trade['jifenbao']=jfb_data_type($trade['fxje'] * TBMONEYBL);
				$trade['jifenbao']=number_format($trade['jifenbao'],2);
				$trade['jifen']=(int)($trade['fxje'] * $this->webset['jifenbl']);
			}
			if(!isset($user['level'])){
				$user['level']=$this->select('user','level','id="'.$user['id'].'"');
			}
			$trade['fxje']=fanli_text($trade['fxje'],$user,$this->webset,1);
			$trade['jifenbao']=fanli_text($trade['jifenbao'],$user,$this->webset,0);
			$trade['jifen']=(int)($trade['fxje'] * $this->webset['jifenbl']);
		}
		...
		if($user['tjr']>0){
			$tjr_user=$this->select('user','*','id="'.$user['tjr'].'"');
		...
		if($trade['id']>0){  //如果是找回订单（前台和后台），带有订单id，从新计算返利值
			...
			if(isset($trade['order_code'])){
				...
				$fxje=fanli_text($fxje,$user,$this->webset,1);
	...
	function tixian($id,$do='yes',$ddback=0,$api_ok='',$api_return=''){
		$tixian=fs('tixian',$this);
		return $tixian->chuli($id,$do,$ddback,$api_ok,$api_return);
	}
	...
	function reg_trade_uid($trade_id,$num_iid,$create_time,$item_title){
		...
		if($this->webset['taoapi']['auto_fanli_plan']['trade_uid']==1){
			$trade_uid=substr($trade_id,-$weishu,$weishu);
	...
	function login($device='pc'){
		...
		if (isset($_POST['sub']) && $_POST['sub'] != '') {
			...
			$dduser = $this -> select('user', 'id,ddusername,email,jihuo', "(ddusername='" . $username . "' or email='" . $username . "' or (mobile <>0 and mobile='" . $username . "' and mobile_test=1)) and ddpassword='" . $md5pwd . "' and del=0");
			$uid = $dduser['id'];
			if ($uid > 0) { // 如果会员存在
				$id = $dduser['id'];
	...
	function register($device='pc'){
		...
		if (isset ($_POST['username']) && $_POST['username'] != '') {
			...
			if($regtype==1){
				...
				$yzm_file_name=dd_crc32(DDKEY).$mobile;
	...
	field3目标数据表获取的字段名
	*/
   	function data_to_data($data,$field1,$table='',$field2='',$field3=''){
		if($field1=='uid' && $table=='' && $field2=='' && $field3==''){
			$table='user';
			$field2='id';
			$field3='ddusername';
		}
		...
		$str=implode(',',$t);
		$data1=$this->select_all($table,$field2.','.$field3,$field2.' in ('.$str.')');
		foreach($data as $k=>$row){
			foreach($data1 as $arr){
				if($arr[$field2]==$row[$field1]){
					unset($arr[$field2]);
