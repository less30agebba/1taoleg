...
class ddTaoapi extends Taoapi{
	...
	function get_et($pid,$pgid){
		...
		$t2=$t+3;
		$url='http://g.click.taobao.com/load?rf='.$rf.'&dr=&pid='.$pid.'&pgid='.$pgid.'&ak=&ttype=1&iframe=false&st='.$t.'%2C'.$t2.'&lan=0%2C1&ciid=44304181548&csid=&curl=&ckeywords=&cbh=361&cbw=1440&re=1440x900&cah=860&caw=1440&ccd=32&ctz=8&chl=9&cja=1&cpl=31&cmm=54&cf=11.3&cb=jsonp_callback_05700072422623634';
		$a = dd_get($url);
	...
	function taobao_tbk_tdj_get($iid,$type=1,$return_type=0,$pid='',$rf=''){
		...
		if(is_array($alimama_plugin) && $alimama_plugin['user_adzone']==1){
			if(!class_exists('plugin_alimama_trade')){
				include(DDROOT.'/plugin/alimama/fun.php');	
			}
			$pid=plugin_alimama_trade::get_pid($pid);
		}
		...
		if(($a['code']==400 || $a=='') && $this->catch_num>=0){
			...
			return $this->taobao_tbk_tdj_get($iid,$type,$return_type,$pid,$rf);
		}
	...
	function do_rebate_order($data){
		...
		foreach($data as $k=>$row){
			...
			if(strpos($row['trade_parent_id'],'+')!==false){
				$row['trade_id_former']=number_format((string)$row['trade_parent_id'],0,'','');
			}
			else{
				$row['trade_id_former']=$row['trade_parent_id'];
			}
			if(strpos($row['trade_id'],'+')!==false){
				$row['trade_id']=number_format((string)$row['trade_id'],0,'','');
			}
			else{
				$row['trade_id']=$row['trade_id'];
			}
			
			$trade=$duoduo->select('tradelist','id,status,uid','trade_id_former="'.$row['trade_id_former'].'" and app_key=0');
			if($trade['id']>0){
				if($trade['status']<5){
					$duoduo->delete('tradelist','trade_id_former="'.$row['trade_id_former'].'" and app_key=0',0);
					if($row['outer_code']=='' && $trade['uid']>0){
						$row['outer_code']=$trade['uid'];
					}
				}
				else{
					unset($data[$k]);
			...
			if($row['outer_code']==''){
				$re=$tradeClass->reg_trade_uid($row['trade_id_former'],$row['num_iid'],$row['create_time'],$row['item_title']);
				if($re['uid']>0){
					$row['outer_code']=$re['uid'];
				}
			}
	...
	function taobao_taobaoke_listurl_get($q,$outer_code){ //S8接口，可以自己拼装url
		...
		$url='https://ai.taobao.com/search/index.htm?key='.rawurlencode($q).'&pid='.$pid.'&commend=all&unid='.$outer_code.'&taoke_type=1';
	    return $url;
	}
	...
	*/
	...
	/*
	└ ext String 可选 {"xx":"xx"}扩展字段JSON格式
	└ logo String 可选 http://m.taobao.com/xxx.jpg口令弹框logoURL
	└ text String 必须 超值活动，惊喜活动多多口令弹框内容
	└ url String 必须 http://m.taobao.com口令跳转url
	└ user_id Number 可选 24234234234 默认值：0 生成口令的淘宝用户ID
	*/
	function taobao_wireless_share_tpwd_create($param){
		extract($param);
		$this->method='taobao.wireless.share.tpwd.create';
		$p=array();
		$p['text']=$text;
		$p['logo']=$logo;
		$p['url']=$url;
		$p['user_id']=99999999999;
		$this->tpwd_param=json_encode($p);
		$TaobaokeData = $this->Send('get',$this->format)->getArrayData();
		if($TaobaokeData['model']!=''){
			return $TaobaokeData['model'];
		}else{
			return '';
		}
	}
	
	function taobao_tbk_spread_get($url){
		$this->method='taobao.tbk.spread.get';
		$p=array('url'=>$url);
		$this->requests=json_encode($p);
		$TaobaokeData = $this->Send('get',$this->format)->getArrayData();
		if($TaobaokeData['results']['tbk_spread'][0]['err_msg']=='OK'){
			return array('s'=>1,'r'=>$TaobaokeData['results']['tbk_spread'][0]['content']);
		}
		else{
			return array('s'=>0,'r'=>$TaobaokeData['results']['tbk_spread'][0]['content']);
		}
	}
	
	function taobao_tbk_dg_item_coupon_get($data){
		if(!isset($data) || $data['adzone_id']==''){
			$a=$this->ApiConfig->taodianjin_pid;
			$a=explode('_',$a);
			$data['adzone_id']=$a[3];
		}
		$this->method='taobao.tbk.dg.item.coupon.get';
		$this->adzone_id=$data['adzone_id'];
		if(!isset($data['q'])){
			$data['q']='';	
		}
		if(!isset($data['cat'])){
		...
		$this->page_no=$data['page_no'];
		$TaobaokeData = $this->Send('get',$this->format)->getArrayData();
		return $TaobaokeData['results']['tbk_coupon'];
	}
	
	function taobao_tbk_ju_tqg_get($data){
		if(!isset($data) || $data['adzone_id']==''){
			$a=$this->ApiConfig->taodianjin_pid;
			$a=explode('_',$a);
			$data['adzone_id']=$a[3];
		...
		$this->adzone_id=$data['adzone_id'];
		$this->fields='click_url,pic_url,reserve_price,zk_final_price,total_amount,sold_num,title,category_name,start_time,end_time';
		...
		$this->page_no=$data['page_no'];
		$TaobaokeData = $this->Send('get',$this->format)->getArrayData();
		return $TaobaokeData['results']['results'];
	}
	
	function taobao_tbk_content_get($adzone_id){
		$this->method='taobao.tbk.content.get';
		$this->adzone_id=$adzone_id;
		$TaobaokeData = $this->Send('get',$this->format)->getArrayData();
		return $TaobaokeData;//['results']['data'];
	}
	...
	└ user_id Number 可选 24234234234 默认值：0 生成口令的淘宝用户ID
	*/
	function taobao_tbk_tpwd_create($param){
		extract($param);
