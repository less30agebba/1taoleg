...
class ddopen{
	...
	function mianshen($tixian_key){
		$this->tixian_key=$tixian_key;
		$this->mianshen=1;
	}
	...
	function sms_send($mobile,$content,$msgset_id=0,$data=array()){//$data 数组包含 sign 群发标记 num 群发总数
		...
		//使用yituo发短信
		if($webset['sms']['type']=='yituo'&&!is_array($mobile)){
			$sms=fs('sms',$duoduo);
			$sms->msgset_id=$msgset_id;
			$row=$sms->send($mobile,$content);
			return $row;
		}
		if(is_array($content)){
	...
	function build_post($parame,$canshu,$type=0){
		...
		//echo $url."<br/>";var_export($context);var_export($canshu);exit;
		...
		if($parame['mod']=='sms' || $parame['mod']=='sms_qunfa'){
			...
			if($parame['mod']=='jifenbao' && $parame['act']=='pay'){
				if($this->mianshen==1){
					...
					$t=time();
					$parame['time']=$t;
					$parame['tixian_key']=md5($this->tixian_key.$t.$this->tixian_key);
				}
