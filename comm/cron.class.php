...
class cron{
	...
	function __construct($duoduo){	
		...
		$this->phplock=new phplock('cron_'.$this->cron_id);
		if($this->phplock->status==0){
...
function check_dd_code($filename){//判断是否采集duoduo数据
	$dd_code_arr=array('cron_caiji.php','cron_mall.php');
	if(in_array($filename,$dd_code_arr)){
		return 1;
	}else{
		return 0;
	}
...
function cron_next_time($row){
	...
	$row['dd']=check_dd_code($row['filename']);
