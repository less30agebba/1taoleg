	...
	function do_item($item,$goods_attribute_arr=array(),$goods_type=array()){
		...
		if(FANLI==1){
			...
			if($item['laiyuan_type']<3){
				$item['fanli']=fanli_text($item['fanli'],$this->dduser,$this->webset,1);
		...
		if(WEBTYPE==0 && ($item['laiyuan_type']==1||$item['laiyuan_type']==2)){
			$item['url']=u('jump','goods',array('iid'=>iid_encode($item['data_id'])));
		}
		else{
		...
		if($item['tg_url']){
			$item['jump']=HTTP.$this->get_bankuai_url($item['code']).'/index.php?mod=jump&act=s8&url='.urlencode(base64_encode($item['tg_url'])).'&iid='.urlencode(iid_encode($item['data_id']));
			$item['wap_jump']=HTTP.$this->get_bankuai_url($item['code']).'/m/index.php?mod=jump&act=index&url='.urlencode(base64_encode($item['tg_url'])).'&id='.$item['id'];
		}
		else{
		...
		$item['tuiguangren_code']='1=1';
		if($item['uid']>0){
			if($item['laiyuan_type']==3){ //商城类型，反馈标签记录推广人
				$item['jump'].='&code='.urlencode('goods|'.$item['uid'].'|'.$item['id']);
			}
			else{ //淘宝类型，记录浏览日志
				$data=array('fuid'=>$item['uid'],'mall'=>2,'code'=>'goods','shuju_id'=>$item['id'],'goods_id'=>$item['data_id']);
				if($item['jump']!=''){
					$item['jump'].='&'.http_build_query($data);
				}
				$item['tuiguangren_code']=http_build_query($data);
			}
		}
