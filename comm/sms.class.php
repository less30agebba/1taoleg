...
class sms{
	...
	function __construct($duoduo){
		...
		$sms=$webset['sms'];
		$type=$sms['type']=='yituo'?"yituo":$sms['type'];
		$user=$sms['yituo_user'];
		$pwd=$sms['yituo_pwd'];
	...
	function send($mobile,$content){
		$session_file=DDROOT.'/data/temp/mobileyzm/'.date('Ymd').'/'.$mobile.'.yzm';
		if(is_file($session_file)){
			$ym_num=file_get_contents($session_file);
		}else{
			$ym_num=0;
		}
		...
		if($ym_num>4){
			$msg['s']=0;
			$msg['r']="操作频繁了，明天再试吧或联系客服";
			return $msg;
		}
		$ym_num++;
		create_file($session_file,$ym_num);
		...
		$content=$content.'回TD退订【'.$this->sign.'】';
		return $this->sms_class->send($mobile,$content);
	}
	//语音验证发送接口
	function sendVoiceCall($mobile,$content){
		$session_file=DDROOT.'/data/temp/mobileyzm/'.date('Ymd').'/'.$mobile.'.yzm';
		if(is_file($session_file)){
			$ym_num=file_get_contents($session_file);
		}else{
		...
		return $this->sms_class->sendVoiceCall($mobile,$content);
	}
	//短信余额接口
...
{ 
	...
	public function sendVoiceCall($sim,$content);
	public function get_balance();
...
class yituo implements sms_jiekou{
	...
	public function sendVoiceCall($mobile,$content){
		return $this->sms_class->sendVoiceCall($mobile,$content);
	}
	public function get_balance(){
		return $this->sms_class->get_balance();
