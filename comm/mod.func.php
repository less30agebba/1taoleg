...
function get_tao_id($url, $tao_id_arr=array()) {
	if(preg_match('#^(http|https)://(item\.taobao\.com)|(detail\.tmall\.com)|(h5\.m\.taobao\.com)|(detail\.m\.tmall\.com)|(detail\.tmall\.hk)|(detail\.m\.tmall\.hk)|(detail\.yao\.95095\.com)#',$url)){
		if(empty($tao_id_arr)){
...
function check_jump_url($url, $uid = 0) {
	...
	elseif(strpos($url,'uland.taobao.com')!==false){
		$lm='quan';
	}
	...
	elseif(strpos($url,'gouwuke.com')!==false){
	    $u='e';
		$lm='yiqifa';
 ...
 * 25号改版，金额区间方法
 *
 * @param integer $commission type=0标示输入的返利集分宝，输出也是集分宝 type=1标示输入的是返利金额，输出也是金额
 * @param array $user 对应会员
 * @param array $webset 网站参数
 * @param integer $type 0表示输出或输入的是集分宝，1表示输出或输入的是金额
 * @return void 区间返利金额
 */
function fanli_text($commission=0,$user=array(),$webset=array(),$type=0){
	if(!isset($webset['fanli_level'])){
		$webset['fanli_level']=-1;
	}
	if($webset['fanli_level']==-1){
		return $commission;
	}
	if($user['level']>=$webset['fanli_level'] && $webset['fanli_level']>=0){
		return $commission;
	}
	if($commission==0){
		return 0;
	...
	if($type==1){
		$commission=$commission*TBMONEYBL;
	}
	$commission=round($commission,2);
	$fanli=0;
	$qujian=explode("\r\n",$webset['qujian']);
	foreach($qujian as $vo){
		if(empty($vo)){
			continue;
		}
		preg_match('/(\d+)-(\d+):(\d+)/',$vo,$match);
		$sprice=round($match[1],2);
		$eprice=round($match[2],2);
		if($sprice<=$commission&&$commission<=$eprice){
			$fanli=floatval($match[3]);
			break;
		}
	}
	if($type==1){
		$fanli=round($fanli/TBMONEYBL,2);
	}
	unset($webset);
	return $fanli;
}
