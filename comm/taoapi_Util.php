...
class Taoapi_Util {
	...
	function fetch($URI) {
		...
		elseif(function_exists('file_get_contents')){
			$a = $this->get_file_get_contents($URI);
			$this->num--;
			if ($a == '' && $this->num > 0) {
				$a = $this->fetch($URI);
			} else {
	...
	"'&o(uml|UML);'",
		...
		$replace = array (
			...
			" ",
			chr(161
		), chr(162), chr(163), chr(169), chr(174), chr(176), chr(39), chr(128), "?", "?", "?", "?", "?", "?", "?");
	...
	function _connect(& $fp) {
		...
		elseif(function_exists('pfsockopen')){
			$fp=pfsockopen($host, $port, $errno, $errstr, $this->_fp_timeout);
		}
	...
	function get_file_get_contents($URI){
