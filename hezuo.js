...
/*template/fenxiao/inc/js/fun.js*/
function b(){
	h = $(window).height();
...
/*template/fenxiao/inc/js/vipduo.js*/
...
function callback(a){
	...
	$.ajax({
		...
		jsonp:"callback",
		success: function(data){
			key_1 = '【<b style="color:#Ff5151;font-family:"微软雅黑"; display:inline-block; max-width:225px; line-height:18px; ;white-space: nowrap;text-overflow: ellipsis;">'+key+'</b>】';
			for(var i in data){	
				data[i]['title'] = '<span style=\"color:#ff5151\">'+data[i]['title']+'</span>';
				str += "<li><a href='index.php?mod=mall&act=view&id="+data[i]['id']+"' target='_blank'><span style='vertical-align:middle;float:left;width:200px;padding-left:20px'><img src=\""+data[i]['img']+"\"width='85px' height='40px' style='line-height:40px;vertical-align:middle;'><span style='margin-left:10px;vertical-align:middle;'>" + data[i]['title'] + "</span></span><span style='vertical-align:middle;line-height:40px;color:#ff3366;margin-right:15px;float:right;width:100px'>最高返利"+data[i]['fan']+"</span><div style='clear:both;'></div></li>";
			}
			...
			//	str += "<li onclick=\"javascript:window.location.href='index.php?mod=paipai&act=list&q="+encodeURIComponent(key)+"'\" class='li_border'><a style='margin-left:10px'> 点击搜" + key_1  + " 相关拍拍宝贝</a></li>";
			//	str += "<div id='s8_tao'><li onclick=\"javascript:window.location.href='index.php?mod=tao&act=view&q="+encodeURIComponent(key)+"'\" class='li_border'><a style='margin-left:10px;;'> 点击去淘宝搜索" + key_1  + " </a></li></div>";
			$('#'+taobaokeytips).html('<ul>'+str+'</ul>');
			$('#'+taobaokeytips).show();
		}
	});
}
/*template/fenxiao/inc/js/taokey.js*/
