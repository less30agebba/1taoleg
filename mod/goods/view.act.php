...
function dd_act(){
	...
	if($id){
		...
		if(WEBTYPE==0){
			if($click_url!=''){
				$jumpurl='?mod=jump&act=s8&iid='.urlencode(iid_encode($goods['data_id'])).'&url='.urlencode($click_url).'&'.$goods['tuiguangren_code'];
				jump($jumpurl);
			}
		}
	}
	elseif($iid){ //如果是淘宝商品，跳转到淘宝模块处理
		if(WEBTYPE==0){
			if($click_url!=''){
				$jumpurl='?mod=jump&act=s8&iid='.urlencode(iid_encode($iid)).'&url='.urlencode($click_url);
				jump($jumpurl);
			}
		}
