...
function act_user_tradelist($pagesize = 10) {
	...
	if ($do == 'taobao') {
		...
		foreach($dingdan as $k=>$row){
			...
			if($row['trade_id_former']==0){
				$dingdan[$k]['trade_id_former']=$dingdan[$k]['trade_id'];
			}
		}
	}
	elseif ($do == 'lost') {
		if (isset ($_GET['q']) && is_numeric($_GET['q']) && $_GET['q']>0) {
			$q = $_GET['q'];
			$dingdan = $duoduo->select_all('tradelist', '*', 'uid=0 and del=0 and trade_id_former = "' . $q . '" limit ' . $frmnum . ',' . $pagesize);
			if(empty($dingdan)){
				$dingdan = $duoduo->select_all('tradelist', '*', 'uid=0 and del=0 and mini_trade_id = "' . substr($q,0,8).substr($q,-4) . '" limit ' . $frmnum . ',' . $pagesize);
			}
			foreach($dingdan as $k=>$row){
