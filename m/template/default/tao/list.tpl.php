...
function index_tpl(){/*<li> <a href="{$url}"> <span class="mu-tmb"><img data-original="{$img}" title="{$title}" class="lazy" src="<?=$dd_tpl_data['loading_img']?>" style="width:100px; height:100px; margin:0 auto;display: block;"> </span> <span style="max-height:3em; overflow:hidden; display:block; font-size:1em">{$title}</span>
        <span class="pricecolor1"><span>原价：￥{$reserve_price}</span></span>
        <span class="pricecolor2">现价：￥{$zk_final_price}<span><?=FANLI_WORD?></span></span>
...
<div class="mc radius">
	<ul class="mu-l2w shuju_load">
		...
		<li> <a href="<?=$row['url']?>"> <span class="mu-tmb"><img data-original="<?=tb_img($row['img'],200)?>" title="<?=$row['title']?>" class="lazy" src="<?=$dd_tpl_data['loading_img']?>" style="width:100px; height:100px; margin:0 auto;display: block;"> </span> <span style="max-height:3em; overflow:hidden; display:block; font-size:1em"><?=$row['title']?></span>
        <span class="pricecolor1"><span style="color:#666">原价：<del>￥<?=$row['reserve_price']?></del></span></span>
        <span class="pricecolor2">现价：￥<?=$row['zk_final_price']?>  <span><?=FANLI_WORD?></span> <!--<span style=" background:#09F;">手机专享</span>--></span>
