...
<p class="line"></p>
<p class="gwcLogin">
<a href="<?=SITEURL?>/?web=1" style="font-size: 1em;" target="_blank">电脑版</a>
<?php if($dduser['id']>0){?>
<a href="<?=wap_l('user')?>" style="font-size: 1em;" >会员中心</a>
<?php }else{?>
<a href="<?=wap_l('user')?>" style="font-size: 1em;" >登录</a>
<?php }?>
<a href="<?=wap_l('help')?>" style="font-size: 1em;">帮助中心</a>
<?php if(app_status()==1){?>
<a href="<?=u('app','phone')?>" style="font-size: 1em;" target="_blank" class="a3"><span id="goods_number">new</span>下载app</a>
<?php }else{?>
<a href="<?=wap_l()?>">首页</a>
<?php }?>
</p>
...
<div class="download-con"<?php if($dd_tpl_data['foot_open']==0){?> style="bottom:0"<?php }?> >
	<div class="down_app" style="display:none">
        ...
        <div class="alogo">
        	<p class="client-name"><?=$dd_tpl_data['tishi_words']?$dd_tpl_data['tishi_words']:'使用客户端拿'.FANLI_WORD1.'更快捷方便'?></p>
        </div>
        <div class="open_now">
        	<a href="<?=$open_app_url==''?u('app','phone'):$open_app_url?>">
...
<script>
$(function(){
	<?php if($dd_wap_class->is_weixin()==1 && $dd_tpl_data['no_weixin']==1){?>
	$('#weixindiv').show();
	<?php }?>
})
</script>
