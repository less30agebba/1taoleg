...
if($_POST['sub']!=''){
	...
	dd_set_cache('bankuai_jd',$tpl_data['bankuai_jd']);
	dd_set_cache('color',$tpl_data['color']);
...
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
    ...
    <tr>
        ...
        <td>&nbsp;<input name="loading_img" type="text" value="<?=$tpl_data['loading_img']?$tpl_data['loading_img']:'m/template/w_chaofan/inc/images/lazy.gif'?>" id="loading_img" class="btn3" style="width:300px" /> <input class="sub" type="button" value="上传预载图片" onclick="javascript:openpic('<?=u('fun','upload',array('uploadtext'=>'loading_img','do'=>'httpurl','sid'=>session_id()))?>','upload','450','350')" /> <span class="zhushi">根据模版预载图片实际尺寸上传，格式不限。默认模板大小：150px*150px，可gif</span>
        </td>
    </tr>
    <!--<tr>
    ...
    <?php
$bankuai_arr=$duoduo->select_2_field('bankuai','code,title','status=1');
$bankuai_arr=array(''=>'全部')+$bankuai_arr;
unset($bankuai_arr['']);
?>
  <tr>
    <td align="right">板块设置：</td>
    <td>&nbsp;京东超返：<?=select($bankuai_arr,$tpl_data['bankuai_jd'],'bankuai_jd')?>&nbsp;&nbsp;&nbsp;&nbsp;<span class="zhushi">显示在首页和列表分类处最后一个，选择板块必须先开启。<a href="<?=u('bankuai','list')?>">设置板块</a></span>
    </td>
  </tr>
  <tr>
