...
appcan && appcan.define('request',function($,exports,module){
  ...
  function ajaxSuccess(data, xhr, settings, deferred) {
      var context = settings.context, status = 'success';
      settings.success.call(context, data, status, xhr);
      if (deferred && typeof deferred.resolveWith!='undefined') {
  ...
  function processCompleteResult(xhr,opcode,status,result,deferred){
      var settings = xhr['settings_'+opcode];
      var dataType = settings.dataType;
      if(status < 0){
          //ajaxError(null,'request error...', xhr, settings, deferred);
		  uexWindow.alert('提醒','网络连接失败','好');
          return;
      }
      if (status == 1) {
          //todo release 
          ...
          ajaxError(null,'error', xhr, settings, deferred);
      }
      xhr.close(opcode);
  }
...
window.appcan && appcan.define('window',function($,exports,module){
    ...
    */
    function open(name,data,aniId,type,dataType,width,height,animDuration){
        ...
        //打开新窗口
        uexWindow.open(name,dataType,data,aniId,width,height,type,animDuration);
    }
    ...
    function setBounce(bounceType,startPullCall,downEndCall,upEndCall,color,imgSettings){
        ...
        if(startPullCall || downEndCall || upEndCall){
            ...
            for(var i in bounceType){
                ...
                uexWindow.setBounceParams(bounceType[i],imgSettings);
    ...
    */
    ...
    function popoverElement(id,url,left,top,name,flag){
        var argObj = null;
        ...
        openPopover(name,0,url,'',left,top,width,height,fontSize,flag,0);
    }
    ...
    */
    function openPopover(name,dataType,url,data,left,top,width,height,fontSize,type,bottomMargin){
        var argObj = null;
        if(arguments.length === 1 && appcan.isPlainObject(name)){
            argObj = name;
            name = argObj['name'];
        ...
        if(uexWidgetOne.platformName && uexWidgetOne.platformName.toLowerCase().indexOf('ios') > -1){
            var args = ['"'+name+'"',dataType,'"'+url+'"','"'+data+'"',left,top,width,height,fontSize,type,bottomMargin];
            var scriptContent = 'uexWindow.openPopover(' + args.join(',') +')';
            evaluateScript('',scriptContent);
            return;
        }
        uexWindow.openPopover(name,dataType,url,data,left,top,width,height,fontSize,type,bottomMargin);
    }
    ...
    */
    function openMultiPopover(popName,content,dataType, left, top, width, height,change, fontSize, flag, indexSelected){
        ...
        uexWindow.openMultiPopover(content,popName,dataType, left, top, width, height, fontSize, flag, indexSelected);
...
window.appcan && appcan.define('frame',function($,exports,module){
    ...
    var appcanFrame = module.exports = {
        ...
        open:function(id,url,left,top,name,index,change,flag){
            var argObj = null;
            if(arguments.length === 1 && appcan.isPlainObject(id)){
                argObj = id;
                id = argObj['id'] || 0;
            ...
            if(appcan.isArray(url)){
                ...
                appWin.openMultiPopover(name || id,
                    url,0,left,top,width,height,change||function(){},fontSize,0,index);
            }
            else{
                appWin.popoverElement(id,url,left,top,name,flag);
            }
        },
