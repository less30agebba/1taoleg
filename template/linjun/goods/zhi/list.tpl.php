    ...
    <!--这里必须定义一个goods_items，滚动加载-->
      <div class="zdm-list zhidemai_goods_list goods_items"id="zhidemaiDiv">
        ...
        <div id="J_zdm_list" class="list-good" style="position:relative">
          <div class="zdm-list-item J-item-wrap item-no-expired" data-id="2" id="post-2"><?php if($row['top']>0){?><i class='zhiding'>置顶</i><?php }elseif($row['is_new']){?><i class='zuixinfabu'>最新发布</i><?php }else{?><i class='lowest-network'>全网最低</i><?php }?><h4 class="t"><a class="J-item-track nodelog click_url" dd-href="?mod=goods&act=view&iid=<?=$row['data_id']?>&id=<?=$row['id']?>&click_url=" data-itemid="<?=$row['data_id']?>" isconvert=1 href="<?=$row['url']?>" target="_blank"><?=$row['title']?></a><?php if($row['discount_price']>0){?><span class="red t-sub">[<?=(float)$row['discount_price']?>元]</span><?php }?></h4>
            <div class="item-img"> <a href="<?=$row['url']?>" dd-href="?mod=goods&act=view&iid=<?=$row['data_id']?>&id=<?=$row['id']?>&click_url=" data-itemid="<?=$row['data_id']?>" isconvert=1 class="click_url J-item-track img nodelog" target="_blank"><?=dd_html_img($row['img'],$row['title'],200)?></a> <a href="<?=$row['url']?>" dd-href="?mod=goods&act=view&iid=<?=$row['data_id']?>&id=<?=$row['id']?>&click_url=" data-itemid="<?=$row['data_id']?>" isconvert=1 class="btn-zdm-goshop J-item-track nodelog click_url" target="_blank">去购买</a> </div>
