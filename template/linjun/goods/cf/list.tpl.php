...
<div class="item-mod list-good">
               	<div>
               		<a href="<?=$row['url']?>" target="_blank"><?=dd_html_img($row['img'],$row['title'],320)?></a>
                	<a href="<?=$row['url']?>" target="_blank"><div class="title"><span class="red"><?=$row['zhe']?>折/</span><?=$row['title']?></div></a>
                </div>
                <div class="money desc">
                    <p class="price"> ¥<strong><?=$row['discount_price']?></strong><del>¥<?=$row['price']?></del></p>
                    <?php if(FANLI==1){?>
                    <p class="fl clearfix"><?=FANLI_WORD?></p>
                    <?php }?>
                </div>
                <a target="_blank" href="<?=$row['url']?>" class="click_url mod-btn <?php if($row['is_starttime']){?>jjstart<?php }else{ ?>ht<?php }?>"></a>
