...
?>
<style type="text/css">
table,table td,table th{border:1px solid #ddd;border-collapse:collapse;height:30px;font-size:16px}
</style>
<div style="height:40px; padding-top:20px; margin:20px 0 0 0; border-radius:5px 5px 0 0; background:#FFFFFF">&nbsp;&nbsp;&nbsp;<a href="<?=u(MOD,ACT)?>" class="u-h2 alist  <?php if($tag=='hezuo'){?>cur<?php }?>" >我的报名信息</a>&nbsp;&nbsp;&nbsp;<a href="<?=u(MOD,ACT,array('tag'=>'goods'))?>" class="u-h2 alist <?php if($tag=='goods'){?>cur<?php }?>">我报名的商品</a>&nbsp;&nbsp;&nbsp;<a href="<?=u(MOD,ACT,array('tag'=>'mall'))?>" class="u-h2 alist <?php if($tag=='mall'){?>cur<?php }?>">我报名的店铺</a>
</div>
  <div class="clearfix"></div>
        <div class="bm_input" style="height:auto;border-radius: 0 0 5px 5px; ">
        <?php if($tag=='hezuo'){?>
			<?php if(!empty($ddgoods)){?>
             <div class="pay_cont" style="margin:20px;">
                ...
                <div class="megas512" style="width:600px"><?=pageft($total,$pagesize,u(MOD,ACT,array('tag'=>$tag)),WJT)?></div>            
             <?php }else{?>
                  <div class="megas512" style="margin-bottom:50px">还没有报名商品，<a href="<?=u('hezuo','baoming')?>" style="border:none;"><strong>提交报名</strong></a></div>
             <?php }?>
