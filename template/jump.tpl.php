...
<head>
    ...
    <style>
        body {
            ...
            color: #5E5E5E;
            font-family: Tahoma, Helvetica, Arial, '微软雅黑 Light', sans-serif;
            font-size: 12px
        }
        ...
        form label.checking {
            padding-left: 18px;
            background: transparent url(<?=TPLURL?>/inc/images/checking.gif) no-repeat;
            margin-left: 5px;
            font-style: italic;
            color: #9C9C9C;
            display: none;
        ...
        form label.validate_right {
            padding-left: 12px;
            background: transparent url(<?=TPLURL?>/inc/images/right.gif) no-repeat;
            margin-left: 5px;
            font-style: italic;
            color: green;
            float: none;
...
<div class="alert_bg" id="alert_fast_log_reg" style="display: block;">
    <div class="alert_box">
        ...
        <div class="alert_top"><a class="alert_close" href="<?= $url ?>"></a>
            <div class="alert_title"><span>
<?php
if ($need_tbnick === 1) {
    $title = '';
} else {
            ...
            <div class="login_reg">
                ...
                <ul class="l_r_menu">
                    <li><a href="javascript:;"
                           class="l_log current">登录<?php if (FANLI == 1) { ?>领取<?= FANLI_WORD1 ?><?php } ?></a></li>
                    <li><a href="javascript:;"
                           class="l_reg">注册<?php if (FANLI == 1) { ?>领取<?= FANLI_WORD1 ?><?php } ?></a></li>
                </ul>
                <div class="l_r_login" id="log">
                    <div class="login">
                        <form action="<?= u('user', 'login') ?>" method="post" onSubmit="return ajaxLogin(this)">

                            <span class="fl">帐号：<br/></span>
                            <input type="text" value="" class="login_name" tabindex="1"
                                   name="username" id="username1"/>
                            <div style="clear:both"></div>
                            <br/>
                            <span class="fl">密码：<br/></span>
                            <input type="password" class="login_pwd" tabindex="2"
                                   name="password" id="password1"/>
                            <div style="clear:both"></div>

                            <?php if ($show_yzm == 1) { ?>
                                <span class="fl">验证：</span><input name="captcha" type="text" id="captcha" tabindex="3"
                                                                  maxlength="4" style="width:70px;"
                                                                  class="login_name"/>&nbsp;&nbsp;<?= yzm() ?>
                                <div style="clear:both"></div>
                            <?php } ?>

                            <font><br/><input type="checkbox" class="checkbox" value="1" name="remember" id="remember"
                                              checked="checked"><label
                                        for="checkbox">记住我<span>（3个月免登陆）</span></label></font>
                            <div style="clear:both"></div>
                            <br/>
                            <input type="hidden" name="sub" value="1"/>
                            <div style="margin-top:10px;">
                                <div style="width: 400px;height: 30px">
                                    <div style="float:left;margin-left:50px; _margin-left:10px"><input type="submit"
                                                                                                       id="loginButton"
                                                                                                       class="button"
                                                                                                       value="登 录"
                                                                                                       tabindex="3"/>
                                ...
                                <div style="float:left;margin-left:20px;width: 317px"><a
                                            href="<?= u('user', 'getpassword') ?>"
                                            class="fo_psw" target="_blank">忘记密码?</a>
                                </div>
                            </div>
                            <div style="clear:both"></div>
                            <div class="sort_info">
                                <?php if ($need_tbnick == 1 && FANLI == 1){ ?>
                                    <div class="login_reg"
                                         style="text-align:center; font-size:14px; padding-left:0px; width:auto">

                                        <p style="padding-bottom:20px; margin:0 auto; line-height:25px; text-align:center;">
                                            亲！ ~ 请先去绑定您的淘宝订单号，以便能快速跟踪您的订单！</p>
                                        <p style="padding-bottom:20px;">
                                            <a href="<?= u('user', 'info', array('do' => 'tbnick')) ?>" class="linehei"
                                               style="margin-right:80px;">去绑定</a>
                                ...
                                <?php if ($act != 's8' && $pic != '') { ?>
                                    ...
                                    <div class="sort_price"
                                         style="color:#F00"><?php if ($act == 'goods' || $act == 'paipaigoods') { ?>价格：
                                            <b class="red price"><?= $price ?></b>元
                                            <br/><?php } ?><?php if ($fan) { ?>最高返：<b
                                                class="red fx"><?= $fan ?></b><?php } ?>
                                    </div>
                                <?php } ?>
                                <p><?php if (FANLI == 1){ ?><label class="n_checked select-privacy"><i
                                                class="icon_select"></i>了解什么是 <a target="_blank" class="black"
                                                                                 href="<?= u('article', 'view', array('id' => 7)) ?>">购物<?= FANLI_WORD1 ?></a><?php } ?><?php if (!$dduser['id']){ ?> 和
                                        <a class="black" href="<?= $url ? $url : $iframe_url ?>">
                                            先购物<?php if (FANLI == 1) { ?>，再<?= FANLI_WORD1 ?><?php } ?>
                                            >></a><?php }else{ ?><a style="float:right" class="blue" href="<?= $url ?>">如果您的浏览器没有自动跳转，<b
                                                    style="color:#F00">请点击这里</b></a></label><?php } ?></p>
                    ...
                    <?php if (count($api) > 0) { ?>
                        <div class="union_login">
                            ...
                            <?php foreach ($api as $row) { ?>
                                <a class="l_<?= $row['code'] ?>"
                                   href="<?= u('api', 'do', array('do' => 'go', 'code' => $row['code'])) ?>"><img
                                            style="margin-bottom:-3px"
                                            src="<?= TPLURL ?>/inc/images/login/<?= $row['code'] ?>_1.gif"/>&nbsp;<?= $row['title'] ?>
                                </a>
                            <?php } ?>
                        </div>
                    <?php } ?>
                        ...
                        <form id="register_form" name="form" action="<?= u('user', 'register') ?>" method="post">
                            <ul class="reg">
                                <table border="0" class="reg_table">
                                    <tr>
                                        <td style="word-break: keep-all"><label class="fl" for="email">电子邮箱：</label>
                                        </td>
                                        <td><input type="text" value="" class="reg_email" id="email" tabindex="4"
                                                   name="email"/></td>
                                    ...
                                    <tr>
                                        ...
                                        <td><input type="password" class="reg_pwd" id="password" tabindex="2"
                                                   name="password"/></td>
                                        <td><label class="field_notice">密码6～16位，区分大小写</label></td>
                                    </tr>
                                    <tr>
                                        <td><label for="password" class="fl">重复密码：</label></td>
                                        <td><input type="password" class="reg_pwd" id="password_confirm" tabindex="3"
                                                   name="password_confirm"/></td>
                                        <td><label class="field_notice">密码6～16位，区分大小写</label></td>
                                    </tr>
                                    <tr>
                                        <td><label for="username" class="fl">用户账号：</label></td>
                                        <td><input type="text" class="reg_name" id="username" tabindex="1"
                                                   name="username"/></td>
                                        <td><label class="field_notice">用户名长度3-15位字母或数字</label><label id="checking_user"
                                                                                                      class="checking">检查中...</label>
                                        </td>
                                    </tr>
                                    <?php if ($webset['user']['need_tbnick'] == 1 && TAOTYPE == 1) { ?>
                                        <tr>
                                            <td><label for="qq" class="fl">淘宝帐号：</label></td>
                                            <td><input type="test" class="reg_name" id="tbnick" tabindex="5"
                                                       name="tbnick"/></td>
                                    ...
                                    <?php if ($webset['user']['need_qq'] == 1 && $default_name == '') { ?>
                                        <tr>
                                            ...
                                            <td><input type="test" class="reg_name" id="qq" tabindex="5" name="qq"/>
                                            </td>
                                    ...
                                    <?php if ($webset['user']['need_alipay'] == 1 && $default_name == '') { ?>
                                        <tr>
                                            ...
                                            <td><input type="test" class="reg_name" id="alipay" tabindex="6"
                                                       name="alipay"/></td>
                                            <td><label class="field_notice">务必填写正确</label><label id="checking_alipay"
                                                                                                 class="checking">检查中...</label>
                                            </td>
                                        </tr>
                                    <?php } ?>
                                    <tr>
                                        ...
                                        <td align="left"> &nbsp;<input name="captcha" type="text" id="captcha"
                                                                       tabindex="7" maxlength="4"
                                                                       class="ddinput"/>&nbsp;&nbsp;<?= yzm() ?></td>
                                    ...
                                    <tr>
                                        ...
                                        <td><input type="hidden" name="sub" value="1"/>
                                            <input type="submit" class="button" name="sub" id="registerButton"
                                                   value="注 册" tabindex="8" id="submits"/></td>
                                        <td>&nbsp;</td>
                                    </tr>
                                </table>
                            </ul>
