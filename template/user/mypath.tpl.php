...
<?php if(FANLI==1){?><span class="biaoqian qingse" <?php if($row['fanli']>0){?>title="购买后返<?=$row['fanli'].$row['unit']?>"<?php }?>><?php if($row['fanli']==0){?>有<?=FANLI_WORD1?><?php }else{?>买后返:<?=$row['fanli']?><?php }?></span><?php }?>
    </div>
    <?php if($row['sell']){?>
    <div class="inblock" style="float:right;">
    	<div class="jy_w_buy">
...
<a class="y-like my-del" title="删除该商品" data_id="<?=$row['id']?>">
