        ...
        <div id="J-zdm-article" class="zdm-article" data-pagename="index">
          ...
          <div id="J_zdm_list">
              <div class="zdm-list-item J-item-wrap item-no-expired" data-id="<?=$goods['id']?>" style="position:relative">
                <h4 class="t"><a class="J-item-track nodelog click_url" target="_blank" href="<?=$goods['jump']?>" dd-href="?mod=jump&act=s8&iid=<?=urlencode(iid_encode($goods['data_id']))?>&<?=$goods['tuiguangren_code']?>&url=" data-itemid="<?=$goods['data_id']?>" isconvert=1><?=$goods['title']?></a></h4>
                <div class="item-img">
                  <a href="<?=$goods['jump']?>" target="_blank" class="J-item-track img nodelog click_url" dd-href="?mod=jump&act=s8&iid=<?=urlencode(iid_encode($goods['data_id']))?>&<?=$goods['tuiguangren_code']?>&url=" data-itemid="<?=$goods['data_id']?>" isconvert=1><?=dd_html_img($goods['img'],$goods['title'],210)?></a>
                  <a href="<?=$goods['jump']?>" target="_blank" class="btn-zdm-goshop J-item-track nodelog click_url" dd-href="?mod=jump&act=s8&iid=<?=urlencode(iid_encode($goods['data_id']))?>&<?=$goods['tuiguangren_code']?>&url=" data-itemid="<?=$goods['data_id']?>" isconvert=1>去购买</a>
                </div>
