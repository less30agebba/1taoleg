	...
	<div class="mainbody1000">
      ...
      <div class="shopitem_main" style="*padding-bottom:15px;">
            	...
            	<span class="like-ico <?=$goods['is_shoucang']==1?'l-active':''?>"></span>
                    	...
                    	<a class="spimng" href="?mod=jump&act=s8&iid=<?=iid_encode($goods['iid'])?>" target="_blank"><img id="goodspic" src="images/310.gif" alt="<?=$goods['title']?>" /></a>
                        <div class="tp_ab">
                    ...
                    <div class="shopitem_main_r" style="position:relative;">
                        ...
                        <?php if(FANLI==1){?>
                        <div class="shopitem_main_r_3" style="margin-top:10px"><?php if($has_fanli*$allow_fanli==0){?><span id="zuigaofan">温馨提示：</span> <img src="<?=TPLURL?>/inc/images/rebate_txt_no.png" alt="无奖励" title="该商品无奖励"><?php }else{?>
                        <p class="rate-text" title="根据淘宝最新规则，查询奖励时将不再显示具体<?=FANLI_WORD1?><?=TBMONEY?>，但奖励仍会正常发放，具体<?=TBMONEY?>以到账时为准">商品<?=FANLI_WORD1?>：</p> 
						<?php }?></div>
                        <?php }?>
                       ...
                       <?php if(FANLI==1){?> <?php if($goods['fanli']>0){?><span class="biaoqian qingse"><?=FANLI_WORD?></span><?php }?> <?php }?>
                            </div>
                        <?php }?>
                        ...
                        <?php if(FANLI==1){?>
                        <div class="shopitem_main_r_5" style="margin-top:10px">温馨提示：虚拟商品如话费，游戏，机票等无<?=FANLI_WORD1?>哦！</div>
                        <?php }?>
                        <div class="shopitem_main_r_4" style="margin-top:15px">

                        <a href="?mod=jump&act=s8&iid=<?=iid_encode($goods['iid'])?>" target="_blank"><img alt="立刻去购买" src="<?=TPLURL?>/inc/images/gomai.gif" /></a> 
