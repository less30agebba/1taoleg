    ...
    <div class="shopright" style=" *margin-bottom:-10px;">
      <div class="biaozhun1" style="float:none;">
        ...
        <div class="shopitem_main" style="*padding-bottom:15px;">
          <div class="shopitem_main_l"> <a class="spimng click_url" dd-href="?mod=jump&act=s8&iid=<?=urlencode(iid_encode($goods['data_id']))?>&<?=$goods['tuiguangren_code']?>&url=" data-itemid="<?=$goods['data_id']?>" isconvert=1 target="_blank" href="<?=$goods['jump']?>" ><img id="goodspic" src="images/310.gif" alt="<?=$goods['title']?>" /></a>
          ...
          <div class="shopitem_main_r" style="position:relative;">
            ...
            <div class="inblock hiddens shopitem_main_r_3" style="margin-top:10px">
              ...
              <?php if(FANLI==1){?><span class="biaoqian qingse" <?php if($goods['fanli']>0){?>title="有返利"<?php }?>><?php if($goods['fanli']>0){?>有返利<?php }?></span><?php }?>
            </div>
            <?php }?>
            <?php if($goods['sell']>0){?>
            ...
            <a target="_blank" class="click_url" dd-href="?mod=jump&act=s8&iid=<?=urlencode(iid_encode($goods['data_id']))?>&<?=$goods['tuiguangren_code']?>&url=" data-itemid="<?=$goods['data_id']?>" isconvert=1 href="<?=$goods['jump']?>" >
            <?php }else{?>
            <a target="_blank" class="click_url" href="<?=$goods['jump']?>" >
            <?php }?>
            <img alt="立刻去购买" src="<?=TPLURL?>/inc/images/n_gomai.gif" /></a>
              <?php if($goods['laiyuan_type']<=2){?>
              <a target="_blank" href="<?=$goods['shop_jump']?>" class="click_url" dd-href="?mod=jump&act=s8&iid=<?=urlencode(iid_encode($goods['data_id']))?>&<?=$goods['tuiguangren_code']?>&url=" data-sellerid="<?=$tao_goods['ds_user_id']?>" isconvert=1><img alt="逛逛掌柜店铺" src="<?=TPLURL?>/inc/images/gozhanggui.gif" /></a>
              <?php }?>
            </div>
           <?php }?>
