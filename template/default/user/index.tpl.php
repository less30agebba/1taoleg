...
if($dduser['txstatus']=='1'){
	...
	if($dduser['money']==0){
		$txstate_msg = "感谢您使用".WEBNAME."，当您购物累积获得".FANLI_WORD1."超过".$webset['tixian']['limit']."元，就可以申请提取。祝您购物愉快！";
...
elseif($sign==0){
    $sign_word='亲！您今天的签到'.FANLI_WORD1.'已经领取完毕，明天继续吧！';
}
            ...
            <?php if($default_pwd!=''){?>
             ...
             <?php include(TPLPATH."/user/notice.tpl.php");?>
                ...
                <div class="adminright_ts1">
                    ...
                    <?php }?>
                    <?php if($caiwu_tip==1){?>
                        <li id="noborder"><p>您还没有设置您的财务信息，这将使您无法<?=FANLI_WORD1?>和兑现，我们建议您立即设置！</p><a href="<?=u('user','info',array('do'=>'caiwu'))?>"><div class="tubiao_shezhi"></div></a></li>
                    <?php }?>
                    <?php if($mobile_tip==1){?>
                        <li id="noborder"><p>您还没验证您的手机号码，我们建议您立即验证！</p><a href="<?=u('user','info',array('do'=>'mobile'))?>"><div class="tubiao_shezhi"></div></a></li>
                    <?php }?>
                    <?php if($tbnick_tip==1){?>
                        <li id="noborder"><p>请绑定您的淘宝订单号，以便给您自动<?=FANLI_WORD1?>！</p><a href="<?=u('user','info',array('do'=>'tbnick'))?>"><div class="tubiao_bangdin"></div></a></li>
                    <?php }?>
                    </ul>
                </div>
                <?php }?>
