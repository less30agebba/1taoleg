...
<?php if($v){?><td><span onClick="delete_tr('tbnick_tr_<?=$k?>')" title="点击删除该账号" style="color:#666; cursor:pointer; margin-left:5px;">删除</span></td><?php }?>
		  <td><?php if($k==0){?><label class="field_notice">登记拿<?=FANLI_WORD1?>用，不会作为其他用途，请放心。<?php if($dd_tpl_data['jiaocheng']['tbnick']!=''){?><a href="<?=$dd_tpl_data['jiaocheng']['tbnick']?>" target="_blank" style="color:#F00">如何填？</a><?php }?></label><?php }?><label id="checking_tbnick_<?=$k?>" class="checking">检查中...</label></td>
		</tr>
    <?php }?>
		<tr>
		  <td height="35">&nbsp;</td>
