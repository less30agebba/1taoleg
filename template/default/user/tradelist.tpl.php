...
if($webset['baobei']['shai_jifen']>0){
	$shai_title.='，'.FANLI_WORD1.$webset['baobei']['shai_jifen'].'积分';
}
if($webset['baobei']['shai_jifenbao']>0){
	$shai_title.='，'.FANLI_WORD1.$webset['baobei']['shai_jifenbao'].TBMONEY;
}
...
$do_arr=array('lost'=>'淘宝订单','malllost'=>'商城订单');
    	...
    	<div class="adminmain">
            ...
            <div class="c_border" style="border-top-style:solid; border-top-width:2px;padding-top:10px;">
                ...
                <div class="admin_table">
                    ...
                    <?php if($do=='taobao'){?>
                      ...
                      <tr>
                        ...
                        <td><div style="overflow:hidden; width:235px; white-space:nowrap" title="<?=$r["item_title"]?>"><?=$r["item_title"]?></div></td>
						...
						?>
                        <td title="<?=$t?>"><?=is_kong(date('Y-m-d',strtotime($t)))?></td>
                        <?php 
						if($r["status"]!=5){
							$s=$tb_status_arr[$r["status"]];
						}
						else{
							if($duoduo->no_pay_trade($r['pay_time'],$r['jifenbao'])==1){
								$s='待结算';
							}
							else{
								$s=$tb_status_arr[$r["status"]];
                    ...
                    <?php if($do=='paipai'){?>
                      <tr>
                        ...
                        <th width="73">金额</th>
                        <th width="43">数量</th>
                        <?php if(FANLI==1){?><th width="59"><?=FANLI_WORD1?></th><?php }?>
                        <th width="80">成交时间</th>
                    ...
                    <?php if($do=='paipailost'){?>
                      <tr>
                        ...
                        <th>宝贝名称</th>
                        <th width="60">金额</th>
                        <?php if(FANLI==1){?><th width="60"><?=FANLI_WORD1?></th><?php }?>
                        <th width="80">成交时间</th>
                        <th width="80">找回订单</th>
                      </tr>
                      <?php foreach ($dingdan as $r){?>
                    ...
                    <?php if($do=='malllost'){?>
                      <tr>
                        ...
                        <th width="80">成交数量</th>
                        <th width="60">单价</th>
                        <?php if(FANLI==1){?><th width="120"><?=FANLI_WORD1?></th><?php }?>
                        <th width="150">成交时间</th>
                ...
                <div class="adminright_yuye">
                    ...
                    <?php }elseif($do=='mall'){?>
                    <p>亲！商城订单大约需要2个月才会核对有效，请耐心等待！<a style="font-family:宋体" href="<?=$dd_tpl_data['jiaocheng']['mall_order']?>" target="_blank">查看详细说明>></a></p>
                    <?php }elseif($do=='paipai'){?>
                    <p>亲！拍拍订单需要确认收货后，<?=WEBNAME?>最晚隔天会收到数据反馈！</p>
                    <?php }elseif($do=='lost' || $do=='checked'){?>
                    <p>亲！没有确认收货的订单不能拿到<?=FANLI_WORD1?>哦！</p>
                ...
                <?php if($webset['taoapi']['freeze_limit']>0){
					if(isset($webset['taoapi']['freeze_limit_user'][$dduser['type']])){
						$webset['taoapi']['freeze_limit']=$webset['taoapi']['freeze_limit_user'][$dduser['type']];
					}
					$w=FANLI_WORD1.'大于'.($webset['taoapi']['freeze_limit']).TBMONEY;
				}?>
                <div class="adminright_yuye">
                    <div class="tishitubiao"></div>
