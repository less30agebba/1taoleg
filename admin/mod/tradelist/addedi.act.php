...
if ($_POST['sub'] != '' && TAOTYPE==1) {
    ...
    if ($id > 0) {
		...
		if(mysql_error()){
			$a=$duoduo->select('tradelist','*','trade_id="'.$post['trade_id'].'"');
			echo "此订单是重复订单，请管理员直接删除<pre>";
			print_r($a);exit;
		}
        $tradelist_data = $duoduo->select('tradelist', '*', 'id=' . $id);
		
		//求关联订单的id
		if($tradelist_data['trade_id_former']>0){
		...
		if (isset($_POST['status'])) {
			if($_POST['status']!=5){
				...
				foreach($ids_arr as $id){
					$comm=$duoduo->select('tradelist','commission','id="'.$id.'"');
					$update['fxje']=fenduan($comm,$webset['fxbl'],$user['type']);
					$update['fxje']=fanli_text($update['fxje'],$user,$webset,1);
					$update['jifenbao']=jfb_data_type($update['fxje'] * TBMONEYBL);
					$update['jifen']=(int)($update['fxje']*$webset['jifenbl']);
					$duoduo->update('tradelist', $update, 'id=' . $id);
				}
				jump('-2', "订单状态修改成功");
			}
        ...
        if ($tradelist_data['status'] != 5) {
			if($_POST['caozuo']==1){
				...
				$duoduo->update('tradelist',$_POST,'id='.$id);
			}
        }
        //已经返现的只能进行退款操作
...
elseif($_POST['sub'] != '' && TAOTYPE==2){
	...
	if($id>0){
		...
		if($do==-1){  //退款订单
			...
			if($_POST['caozuo']==2){
				...
				if($_POST['shuoming']){
					...
					$field_arr['see']=0;
					$field_arr['senduser']=0;
					$field_arr['uid']=$user['id']>0?$user['id']:0;
					$field_arr['title']="订单认领失败";
			...
			if(($trade['status']==5||$post['status']==5) &&$post['uid']>0){
		...
		$_POST['mini_trade_id']=substr($_POST['trade_id'],0,8).substr($_POST['trade_id'],-4,4);
		$_POST['trade_id_former']=preg_replace('/_\d+$/','',$_POST['trade_id']);
		$_POST['outer_code']=empty($user['id'])?'':$user['id'];
		$_POST['category_id']=0;
		if($user['id']>0){
			$_POST['uid']=$user['id'];
		}
