...
if(isset($_POST['sub'])){
	...
	if(($webset['sms']['type']=='yituo')){
		$ddsms=fs('sms',$duoduo);
		$balance=$ddsms->get_balance();
		$balance_txt='自定义通道短信（<b>'.$balance['r'].'</b>条）；';
	}
	if($webset['sms']['pwd']!=''){
		$ddopen=fs('ddopen');
		$ddopen->sms_ini($webset['sms']['pwd']);
		...
		if($a['s']==1){
			$sms_tip='<span style="color:#060">'.$balance_txt.'云通知短信（<b>'.$a['r']['sms'].'</b>条）；群发短信（<b>'.$a['r']['sms_qunfa'].'</b>条）</span>';
