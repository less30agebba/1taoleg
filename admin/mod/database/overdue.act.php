		...
		if($code<count($code_arr)){
			...
			if($buzhou==1){
                ...
                foreach($goods as $k=>$row){
                    $ids[] = $row['data_id'];
                    $butongguo[$row['data_id']]=$row['id'];
                }
                $ids = implode(',', $ids);
                $data=$ddTaoapi->taobao_tbk_item_info_get($ids);
                foreach($data as $row){
                    $num_iid = (string)$row['num_iid'];
                    unset($butongguo[$num_iid]);
                }
			}elseif($buzhou==2){
				$str='';
				$aa=array();
			...
			if($buzhou==3){
				...
				if($butongguo){
					if($auto_del==0){
					    foreach($butongguo as $row){
                            $duoduo->update('goods',array('xiajia'=>'1'),'id='.$row);
                        }
					}else{
                        foreach($butongguo as $row) {
                            $duoduo->delete('goods', 'id='.$row);
                        }
					}
				}		
			}
