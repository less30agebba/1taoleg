...
if($step==0){
	$word='第一步：导航检测&nbsp;&nbsp;(检测关键词“淘宝”，“返现”，“现金”，“%”，“返”，“返利”)';
	//导航表nav
	$jiance_arr=array('淘宝','返现','现金','提现','%','返','返利');
	...
	foreach($jiance_arr as $v){
		$article_arr=$duoduo->select_all('article','*','`title` like "%'.$v.'%" or `content` like "%'.$v.'%"');
		foreach($article_arr as $r){
	...
	//seo表seo
	foreach($jiance_arr as $v){
		$seo_arr=$duoduo->select_all('seo','*','`title` like "%'.$v.'%" or `keyword` like "%'.$v.'%" or `desc` like "%'.$v.'%" or `label` like "%'.$v.'%"');
		foreach($seo_arr as $r){
