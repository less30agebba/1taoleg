...
<form action="index.php?mod=<?=MOD?>&act=<?=ACT?>" method="post" name="form1" class="myself">
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
  ...
  <tr>
    ...
    <td>&nbsp;<input name="back_url" type="text" id="back_url" value="<?=$row['back_url']?>" style="width:300px"/> <span class="zhushi"><?=SITEURL?>/api/qq/callback.php 必须和QQ申请时填写的完全一样</span></td>
  </tr>
  <?php }?>
  <tr>
    <td align="right">标示代码：</td>
    <td>&nbsp;<input name="code" type="text" id="code" value="<?=$row['code']?>" <?php if($row['code']!=''){?> readonly <?php }?>/> <span class="zhushi">注意：填写后不可修改</span></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <table border="0">
  <tr>
    <td><textarea name="<?=$row['code']?>_meta" id="meta" style="width:300px; height:100px"><?=$webset[$row['code'].'_meta']?></textarea></td>
...
$('form[name=form1]').submit(function(){
	...
	if($('#meta').attr('name')=='_meta'){
		$('#meta').attr('name',code+'_meta');	
	}
