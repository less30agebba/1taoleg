...
<form action="" method="get">
  <table cellspacing="0" width="100%" style="border:1px  solid #DCEAF7; border-bottom:0px; background:#E9F2FB">
    <tr>
      <td>&nbsp;<img src="images/arrow.gif" width="16" height="22" align="absmiddle" />&nbsp;
      <?php if(TAOTYPE==1){?>
      <a href="<?=u(MOD,'import')?>" class="link3">[导入订单]</a>
      <?php }else{?>
      <a href="<?=u(MOD,'report')?>" class="link3">[获取订单]</a>
      <?php }?>
      &nbsp;<a href="<?=u(MOD,'addedi')?>" class="link3">[添加订单]</a>&nbsp;<?php if($reycle==1){?><a href="<?=u(MOD,ACT)?>" class="link3">[返回列表]</a><?php }else{?><a href="<?=u(MOD,ACT,array('reycle'=>1))?>" class="link3">[回收站]</a><?php }?> </td>
      <td align="right">
      <label>近期订单<input type="checkbox" name="jinqi" value="1" <?php if($_GET['jinqi']==1){?> checked="checked"<?php }?>/></label>
...
<form name="form2" method="get" action="" style="margin:0px; padding:0px">
  <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
    ...
    <tr>
      ...
      <?php }?>
      <?php if($r['uid']>0){?>
      <a style="cursor:pointer" onclick="huanren(<?=$r['id']?>)">换</a>
      <?php }?>
      <?php if($r['uid']>0 && TAOTYPE==1){?>
      <!--<a style="cursor:pointer" onclick="cha(<?=$r['id']?>)">查</a>-->
      <?php }?>
      </td>
    </tr>
    <?php }?>
...
<script>
function huanren(id){
	username = prompt("请输入会员名:","");
	if (username != null){
		$.get('?mod=tradelist&act=huanren&id='+id+'&username='+urlencode(username)+'&r='+Math.round(Math.random()*1000),function(data){
			if(data==1){
				alert('更改成功');
				 history.go(0);
			}
...
function cha(id){
	$.get('?mod=tradelist&act=cha&id='+id+'&r='+Math.round(Math.random()*1000),function(data){
		alert(data);
	});
}
</script>
