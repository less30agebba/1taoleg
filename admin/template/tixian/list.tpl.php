...
<table cellspacing="0" width="100%" style="border:1px  solid #DCEAF7; border-bottom:0px; background:#E9F2FB">
      ...
      <table id="listtable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
					...
					foreach ($row as $r){
						...
						$a=(int)$duoduo->select('user','id','id<>"'.$r['uid'].'" and alipay="'.$r['code'].'"');
						if($a>0){
							$chong=$a;
						}
						else{
					...
					?>
					  <tr>
						...
						<td class=b><?=$tool_arr[$r['tool']].'：'.$r["code"]?><?php if($chong>0){?><a style="color:#F00; text-decoration:underline" href="<?=u('user','addedi',array('id'=>$chong))?>">[有重复]</a><?php }?></td>
