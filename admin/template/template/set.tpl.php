...
if(!iswriteable(DDROOT.'/data/json/tpl')){
	$tpl_write_tip=1;
}
else{
	$tpl_write_tip=0;
...
if($_GET['tpl']!=''){
	$duoduo->set_webset('MOBAN',$_GET['tpl']);
	jump(-1,'设置完成');
}
...
function get_tpl_info(){
	...
	foreach($a as $v){
		if(file_exists($v.'/tpl_info')){
			...
			$b['tpl']=str_replace(DDROOT.'/template/','',$v);
			if(strpos($b['tpl'],'wap_')===0){
				continue;
			}
			$b['img']='../template/'.$b['tpl'].'/tpl_info/index.jpg';
...
$url=DD_YUN_URL.'/?m=api&a=tuijian&type=2&siteurl='.urlencode(get_domain(URL)).'&banben='.$banben;
...
<?php if($tpl_write_tip==1){?>
<div class="explain-col" style="color:#F00; font-size:14px">data/json/tpl目录没有可写权限，请先设置改目录可写</div>
<?php }?>
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
      <tr>
        ...
        <div class="tpl" title="选择模板"><?php if($row['status']=='abled'){?><div class="abled">使用中</div><?php }?><img onclick="setTpl('<?=$row['tpl']?>')" src="<?=$row['img']?>" /><div class="p"><?=$row['title']?></div><div class="p"><span class="<?=$row['status']?>"><?=$row['status_msg']?></span>&nbsp;&nbsp;<a href="<?=u(MOD,'admin_set',array('title'=>$row['title'],'tpl'=>$row['tpl'],'tpl_type'=>'pc'))?>">管理</a><?php if($row['jieshao']==1){?>&nbsp;&nbsp;<a href="../template/<?=$row['tpl']?>/tpl_info/jieshao.html">详细介绍</a><?php }?></div></div>
        <?php }?>
        </td>
      </tr>
      <tr>
        ...
        <?php }?>
        <div class="tpl">
        <a href="<?=DD_YUN_URL?>/index.php?m=bbx&a=index&type=2"><img src="../images/gengduo.png"/></a>
        <div class="p"><a href="<?=DD_YUN_URL?>/index.php?m=bbx&a=index&type=2">更多模版 去看看</a></div>
        </div>
        </td>
      </tr>
    </table>
