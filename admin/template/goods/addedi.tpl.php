...
$(function(){
	...
	$('#quan_stime').calendar({format:'yyyy-MM-dd'}); 
	$('#quan_etime').calendar({format:'yyyy-MM-dd'});
})
</script>
...
<tr>
  ...
  <tr>
   ...
   <tr class="" style="background-color:#f7f9fc;"><td colspan="2">&nbsp;&nbsp;&nbsp;&nbsp;优惠卷信息</td></tr>
  <tr>
    <td align="right">领取地址：</td>
    <td>&nbsp;<input name="lq_url" type="text" id="lq_url" value="<?=$row['lq_url']?>" style="width:600px" /><span class="zhushi">可空，填写手机领取地址<?php if($row['lq_url']){?>，<a href="<?=$row['lq_url']?>" id="shop_url" target="_blank">去查看</a><?php }?></span></td>
  </tr>
  <tr>
    <td align="right">券后价：</td>
    <td>&nbsp;<input name="quan_price" type="text" id="quan_price" value="<?=$row['quan_price']?>" style="width:50px" /></td>
  </tr>
  <tr>
    <td align="right">优惠券：</td>
    <td>&nbsp;满：<input name="price_man" type="text" id="price_man" value="<?=$row['price_man']?>"  style="width:50px"/> 立减 <input name="price_jian" type="text" id="price_jian" value="<?=$row['price_jian']?>"  style="width:50px"/></td>
  </tr>
  <tr>
    <td align="right">优惠券剩余：</td>
    <td>&nbsp;<input name="quan_surplus" type="text" id="quan_surplus" value="<?=$row['quan_surplus']?>" style="width:50px" /></td>
  </tr>
  <tr>
    <td align="right">优惠券总数：</td>
    <td>&nbsp;<input name="quan_total" type="text" id="quan_total" value="<?=$row['quan_total']?>" style="width:50px" /></td>
  </tr>
 <tr>
    <td align="right">开始时间：</td>
    <td>&nbsp;<input name="quan_stime" type="text" id="quan_stime" style="width:140px" value="<?php if(empty($row['id'])){echo date('Y-m-d');}else{echo date('Y-m-d',$row['quan_stime']?$row['quan_stime']:'');}?>" /></td>
  </tr>
  <tr>
    <td align="right">结束时间：</td>
    <td>&nbsp;<input name="quan_etime" type="text" id="quan_etime" style="width:140px" value="<?=$row['quan_etime']?date('Y-m-d',$row['quan_etime']):''?>" /></td>
  </tr>
