...
<table cellspacing="0" width="100%">
        <tr>
          ...
          <td width="480" class="tip">状态信息：<?=$sms_tip?> </td>
          <?php if(DLADMIN!=1){?>
...
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
  ...
  <tr>
    <td width="150px" align="right">自定义通道：</td>
    <td>&nbsp;<?=html_radio(array(0=>'关闭','yituo'=>'开通'),$webset['sms']['type'],'sms[type]')?> <span class="zhushi">单条群发是可以选择自定义通道发送，群发短信必须使用云平台发送</span></td>
  </tr>
  <tbody id="yituo" <?php if(empty($webset['sms']['type'])){?>style="display:none"<?php }?>>
  <tr class="smsopen">
    <td align="right">通道账号：</td>
    <td>&nbsp;<input id="sms_yituo_user" name="sms[yituo_user]"  style="width:80px" value="<?=$webset['sms']['yituo_user']?>" /> <span class="zhushi">自定义通道最低购买2000元短信且提供公司资质</span></td>
  </tr>
  <tr class="smsopen">
    <td align="right">通道密码：</td>
    <td>&nbsp;<input id="sms_yituo_pwd" name="sms[yituo_pwd]" style="width:80px" value="<?=$webset['sms']['yituo_pwd']?>" /><span class="zhushi">联系客服QQ索取：800018982</span></td>
  </tr>
  </tbody>
  <tr class="smsopen">
...
<script>
$(function(){
	$('input[name="sms[type]"]').click(function(){
		if($(this).val()=='yituo'){
			$("#yituo").show();
		}else{
			$("#yituo").hide();
