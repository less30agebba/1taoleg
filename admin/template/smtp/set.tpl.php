...
$(function(){
	$('#testEmail').click(function(){
		...
		var port=$('input[name="smtp[port]"]:checked').val();
		$sub=$(this);
		...
		if(smtphost=='' || smtpuser=='' || smtppwd=='' || test_email==''){
			...
			$.post('<?=u('smtp','set')?>',{'smtphost':smtphost,'smtpuser':smtpuser,'smtppwd':smtppwd,'test_email':test_email,'type':type,'port':port,'title':test_title,'html':test_content},function(data){
			    data=parseInt(data);
				if(data==0){
				    alert('邮件发送失败！');
				}
...
<div class="explain-col">某些邮箱用接口发送邮件会屏蔽某些词语造成发送失。
<br/><?php if(!function_exists('socket_create')){?><b style="color:#F00">您的空间不支持socket_create函数，联系空间商开启，更好的支持邮件发送。</b><?php }else{?>socket_create有效<?php }?>
<br/><?php if(!extension_loaded("openssl")){?><b style="color:#F00">您的空间不支持openssl，联系空间商开启，更好的支持邮件发送</b><?php }else{?>openssl有效<?php }?>
</div>
...
<table id="addeditable" border=1 cellpadding=0 cellspacing=0 bordercolor="#dddddd">
  ...
  <tr>
    <td width="115px" align="right">发送端口：</td>
    <td>&nbsp;<?=html_radio(array(25=>25,465=>465),$webset['smtp']['port'],'smtp[port]')?> <span class="zhushi">默认25端口，如果发送失败尝试465端口。</span></td>
  </tr>
  <tr>
