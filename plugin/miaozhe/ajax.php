...
switch($act){
	...
	case 'malls':
		...
		$malls=$mall_class->select($where." order by sort=0 asc,sort asc,id desc limit ".$num);
	...
	case 'callback_search':
		...
		$mid_arr = $duoduo->select_all($table_name,'title,fan,id,img','title like "%'.$q.'%" and is_search=0 ');
		if($q==''){
			$mid_arr = array();
		}
